...
<div class="vtable">
    ...
    <{foreach from=$all_content item=data}>
        <ul>
            ...
            <li class="vm"><span class="label"><{$data.group_id}></span><{$data.group_name}></li>
            <li class="vcell"><{$smarty.const._TAD_FUNCTION}></li>
            <li class="vm"><a href="javascript:delete_tad_login_config_func(<{$data.config_id}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>
                <a href="<{$action}>?op=tad_login_config_form&config_id=<{$data.config_id}>" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a></li>
        </ul>
    <{/foreach}>
</div>
