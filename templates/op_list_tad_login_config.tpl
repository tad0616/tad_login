    ...
    <{foreach from=$all_content item=data}>
        <ul>
            ...
            <li class="vm"><a href="javascript:delete_tad_login_config_func(<{$data.config_id}>);" class="btn btn-sm btn-xs btn-danger"><i class="fa fa-trash" aria-hidden="true"></i> <{$smarty.const._TAD_DEL}></a>
                <a href="<{$action|default:''}>?op=tad_login_config_form&config_id=<{$data.config_id}>" class="btn btn-sm btn-xs btn-warning"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>  <{$smarty.const._TAD_EDIT}></a></li>
        </ul>
    <{/foreach}>
</div>
...
<div class="text-center" style="margin: 30px;">
    <a href="<{$action|default:''}>?op=tad_login_config_form" class="btn btn-info"><i class="fa fa-plus-square" aria-hidden="true"></i>  <{$smarty.const._TAD_ADD}></a>
</div>
